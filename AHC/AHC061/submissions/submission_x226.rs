...
fn main() {
    unsafe {
        // x226: Hybrid - x213 base (Bayes) + MacroRoute for M=3,4
        env::set_var("AHC_X04_ALLOWED_M", "3,4");

        // M=4 Champion x58 params (default)
        ...
        // M=3: MacroRoute with medium beam
        env::set_var("AHC_X04_PHASE_CUTOFF_M3", "0.85");
        ...
        env::set_var("AHC_X04_BEAM_WIDTH_FAST_M3", "6");
        env::set_var("AHC_X04_BEAM_WIDTH_SLOW_M3", "7");
        env::set_var("AHC_X04_BRANCH_WIDTH_M3", "2");
    }
    run_with_strategy_bayes(StrategyMode::MacroRoute);
}
        ...
        pub mod ahc061_solver {
            ...
            mod x04_macro_route {
                ...
                pub(super) fn choose_move_x04_macro_route(
                    ...
                    // Apply beam search only for M values allowed by AHC_X04_ALLOWED_M (default: M=4 only)
                    ...
                    let plan_len_fast = env_u64("AHC_X04_PLAN_LEN_FAST", 7, 3, 10) as usize;
                    let plan_len_slow = env_u64("AHC_X04_PLAN_LEN_SLOW", 8, 3, 10) as usize;
                    let beam_width_fast = env_u64("AHC_X04_BEAM_WIDTH_FAST", 5, 2, 8) as usize;
                    let beam_width_slow = env_u64("AHC_X04_BEAM_WIDTH_SLOW", 6, 2, 8) as usize;
                    let branch_width = env_u64("AHC_X04_BRANCH_WIDTH", 3, 1, 4) as usize;
                    ...
                    let budget_ms = (per_turn * 96 / 100).max(5);
                    ...
                    // Iterative deepening: loop with progressively wider beams until deadline
                    {
                        ...
                        while Instant::now() < deadline && deep_beam <= 20 {
            ...
            const BAYES_DIM: usize = 5; // w0,w1,w2,w3 + eps
            ...
            impl BayesGrid {
                fn new(rng: &mut FastRng) -> Self {
                    ...
                    let anchor = [0.64, 0.64, 0.64, 0.64, 0.30];
                ...
                fn posterior_mean(&self) -> ([f64; 4], f64) {
                    let max_lw = self
                        .log_weights
                        .iter()
                        .zip(self.active.iter())
                    ...
                    if sum_w > 0.0 {
                        ...
                        return ([0.64, 0.64, 0.64, 0.64], 0.30);
                    }
