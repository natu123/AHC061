...
fn main() {
    unsafe {
        env::set_var("AHC_X04_SOFT_TIME_MS", "1800");
        ...
        env::set_var("AHC_X04_UPGRADE_WEIGHT", "0.72");
        env::set_var("AHC_X04_UPGRADE_WEIGHT_HIGH_U", "1.40");
    }
    ...
    pub(crate) mod crates {
        pub mod ahc061_solver {
            ...
            mod x04_macro_route {
                ...
                fn macro_route_elapsed_ms() -> u128 {
                ...
                fn beam_route_score(
                    ...
                    let first_scores =
                        crate::__cargo_equip::crates::ahc061_solver::calc_scores(game, state);
                    let first_max_ai_i64 = first_scores
                        .iter()
                        .skip(1)
                        .copied()
                        .max()
                        .unwrap_or(1)
                        .max(1);
                    let mut first_is_leader = vec![false; game.m];
                    for p in 1..game.m {
                        if first_scores[p] == first_max_ai_i64 {
                            first_is_leader[p] = true;
                        }
                    }
                        ...
                        for node in &beam {
                            ...
                            my_cands.sort_by(|&a, &b| {
                                let la = crate::__cargo_equip::crates::ahc061_solver::evaluate_local_move(
                                    ...
                                    cur,
                                    &is_leader,
                                ) + 18.0 * (manhattan(cur, target) as f64 - manhattan(a, target) as f64);
                                let lb = crate::__cargo_equip::crates::ahc061_solver::evaluate_local_move(
                                    ...
                                    cur,
                                    &is_leader,
                                ) + 18.0 * (manhattan(cur, target) as f64 - manhattan(b, target) as f64);
                ...
                pub(super) fn choose_move_x04_macro_route(
                    ...
                    let soft_time_ms = env_u64("AHC_X04_SOFT_TIME_MS", 0, 0, 20_000) as u128;
                    if soft_time_ms > 0 && macro_route_elapsed_ms() >= soft_time_ms {
                        return x06_expert_switch_hybrid::choose_move_x06_expert_switch(
                            game, state, models,
                        );
                    }

                    let allow_all_m = env_bool("AHC_X04_ALLOW_ALL_M", false);
                    let disable_phase_cutoff = env_bool("AHC_X04_DISABLE_PHASE_CUTOFF", false);

                    if game.m != 4 && !allow_all_m {
                        return x06_expert_switch_hybrid::choose_move_x06_expert_switch(
                            game, state, models,
                        );
                    }
                    ...
                    let mut target_eval = env_u64("AHC_X04_TARGET_EVAL", 4, 1, 8) as usize;
                    let mut candidate_cap = env_u64("AHC_X04_CANDIDATE_CAP", 7, 4, 20) as usize;
                    let mut plan_len_fast = env_u64("AHC_X04_PLAN_LEN_FAST", 6, 3, 10) as usize;
                    let mut plan_len_slow = env_u64("AHC_X04_PLAN_LEN_SLOW", 7, 3, 10) as usize;
                    let mut beam_width_fast = env_u64("AHC_X04_BEAM_WIDTH_FAST", 4, 2, 8) as usize;
                    let mut beam_width_slow = env_u64("AHC_X04_BEAM_WIDTH_SLOW", 5, 2, 8) as usize;
                    ...
                    let upgrade_weight = if game.u >= 3 && upgrade_weight_high_u >= 0.0 {
                    ...
                    if soft_time_ms > 0 {
                        let elapsed = macro_route_elapsed_ms();
                        let remain_ratio = if elapsed >= soft_time_ms {
                            0.0
                        } else {
                            (soft_time_ms - elapsed) as f64 / soft_time_ms as f64
                        };
                        if remain_ratio < 0.10 {
                            return x06_expert_switch_hybrid::choose_move_x06_expert_switch(
                                game, state, models,
                            );
                        } else if remain_ratio < 0.20 {
                            candidate_cap = candidate_cap.min(5);
                            target_eval = target_eval.min(3);
                            plan_len_fast = plan_len_fast.min(4);
                            plan_len_slow = plan_len_slow.min(5);
                            beam_width_fast = beam_width_fast.min(3);
                            beam_width_slow = beam_width_slow.min(4);
                        } else if remain_ratio < 0.35 {
                            candidate_cap = candidate_cap.min(6);
                            target_eval = target_eval.min(4);
                            plan_len_fast = plan_len_fast.min(5);
                            plan_len_slow = plan_len_slow.min(6);
                            beam_width_fast = beam_width_fast.min(4);
                            beam_width_slow = beam_width_slow.min(5);
                        }
                    }

                    if phase_now > phase_cutoff && !disable_phase_cutoff {
                        return x06_expert_switch_hybrid::choose_move_x06_expert_switch(
                            game, state, models,
                        );
                    }
