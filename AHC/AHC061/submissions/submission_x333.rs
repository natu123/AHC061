...
fn main() {
    // x333: Adaptive Algorithm Selection
    //
    // x332 findings:
    //   M<=4, U=2-3: expectimax excels (+9% to +49% vs x323)
    //   U=1: expectimax catastrophic (-8% to -68%)
    //   M>=5: expectimax catastrophic (-25% to -53%)
    //   M=4, U>=4: expectimax bad (-38% to -47%)
    //
    // Strategy: use expectimax for M<=4,U=2-3; beam/dispatch for everything else.
    //
    // Expected improvement on affected seeds:
    //   Seed 0 (M=4,U=3): 151,289 vs 138,801 (+9%)
    //   Seed 6 (M=2,U=3): 178,655 vs 119,792 (+49%)
    //   Seed 12 (M=3,U=3): 180,955 vs 147,567 (+23%)
    //   Seed 17 (M=4,U=2): 206,135 vs 146,433 (+41%)

    unsafe {
        // Time management
        env::set_var("AHC_TIME_LIMIT_MS", "2000");

        // Expectimax params
        env::set_var("AHC_X332_DEPTH", "3");
        env::set_var("AHC_X332_DEPTH_M2_U3", "4"); // M=2,U=3: 1 opponent + moderate U -> depth 4 safe
        env::set_var("AHC_X332_BRANCH_TOP", "8");
        env::set_var("AHC_X332_BRANCH_DEEP", "4");

        // Beam params (for non-expectimax paths)
    ...
    pub(crate) mod crates {
        pub mod ahc061_solver {
                    ...
                    for x in 0..game.n {
                        for y in 0..game.n {
                            ...
                            if owner == -1 {
                                ...
                                w = upgrade_weight * v * (game.u - level) as f64 / game.u as f64;
                            }
                            if w > 0.0 {
                ...
                pub(super) fn choose_move_x04_macro_route(
                    ...
                    if let Some(dl) = deadline {
                        ...
                        if iter_deepen == 1 {
                            ...
                            let re_eval_count = 3.min(candidate_cap);
            ...
            mod x89_level_fortress {
                /// x89: Level-First Territory Fortress
                ///
                /// 根本的な発想転換:
                /// - x47までの全戦略は「領土拡張」中心で、レベル上げは副次的
                /// - top-1 (S0/SA≈200) は「高V地のレベル最大化 + 防御壁」で盤面支配を達成
                /// - level≥2 にすれば AI が攻撃しても弾かれる（level-1 しか奪取できない）
                /// - AI は eps∈[0.1,0.5] で非効率 → 早期に防御壁を作れば負けない
                ///
                /// 戦略:
                /// 1. 序盤: 高V中立セルを素早く確保
                /// 2. 中盤前半: 高Vセルを level≥2 に上げて防御壁化（最重要）
                /// 3. 中盤後半: 防御壁を維持しながら拡張
                /// 4. 終盤: 残りターンでレベル最大化
                use crate::__cargo_equip::crates::ahc061_solver::{
                ...
                /// セルの「防御価値」を計算
                /// level≥2 なら AI に奪われない → 防御完了
                /// level==1 なら脆弱 → 早急にレベル上げが必要
            ...
            mod x174_chokudai {
                ...
                /// BFS for player's reachable cells (same as lib.rs but self-contained)
                ...
                /// Fast AI prediction: just pick the cell with max w·features for each AI
                fn predict_ai_top1(
                    game: &Game,
                    state: &State,
                    models: &[AiModel],
                ...
                /// Simulate one turn: player moves + AI moves
                ...
                /// Compute S0 and SA (max opponent score)
                ...
                /// 評価関数: S0/SA比 + 領土品質 + Upgrade効率 + フロンティア
                ...
                /// Score a move candidate (fast, O(1) per move)
            ...
            mod x305_full_horizon {
                ...
                /// Deterministic AI move using full BFS + model weights (argmax).
                fn ai_best_move(
                    game: &Game,
                    state: &State,
                    player: usize,
                ...
                /// Phase-aware greedy move for player0 in forward simulation.
                /// Includes distance penalty to avoid oscillation.
                ...
                /// Quick heuristic for candidate pre-filtering.
                ...
                /// Simulate remaining game with full BFS AI + greedy player0.
            ...
            /// Initialise the game timer (called once at program start).
            pub(in crate::__cargo_equip::crates::ahc061_solver) fn init_game_timer() {
                GAME_TIMER.get_or_init(Instant::now);
            }

            /// Return the time budget (ms) available for the current turn.
            /// Returns 0 when time management is disabled (AHC_TIME_LIMIT_MS not set).
            ...
            /// Check if the game timer has exceeded the total limit.
            ...
            /// BFS distance map from player's current position through own territory.
            /// Returns dist[x][y] = minimum steps to reach (x,y), or usize::MAX if unreachable.
            ...
            /// Zone-aware absolute score: rewards territory control within the value zone.
            /// AHC_ZONE_SCORE_W controls the bonus weight (default 0 = disabled).
            /// Bonus = zone_w * sum(v[x][y] * level[x][y]) for own cells in the zone.
            ...
            /// Compute the densest KxK zone on the value grid.
            /// Returns a 2D bool mask: true = cell is in the high-value zone.
            /// Zone size is env-configurable (default 4).
            ...
            /// Opponent-aware zone: densest KxK window penalized by opponent presence.
            /// AHC_ZONE_OPP_PENALTY controls penalty per opponent cell in/adjacent to window (default 0).
            ...
            /// Dual zone: top-2 non-overlapping densest KxK windows.
            /// AHC_ZONE_DUAL=1 enables: second zone must overlap <=50% with first.
            ...
            /// Proximity-weighted zone: KxK window maximizing value / distance from own territory.
            /// Nearby high-value areas are preferred over distant ones.
            /// AHC_ZONE_PROX_DECAY controls distance decay (default 0.15).
            ...
            /// Global cache for value zone (computed once per game)
            static VALUE_ZONE: OnceLock<Vec<Vec<bool>>> = OnceLock::new();

            /// Get or compute the value zone for evaluate_local_move zone bonus
            ...
            /// Score function that includes denial awareness and upgrade potential.
            /// Augments absolute_score with:
            /// - Leader vulnerability penalty (states where leader has many level-1 cells are better for us)
            /// - Own upgrade value bonus (states where we have high-level cells to upgrade further)
            /// - Leader upgrade potential penalty (states where leader has high upgrade potential are worse)
            ...
            /// Fixed denial score: penalizes remaining upgrade potential instead of rewarding it.
            /// This creates proper incentive to upgrade cells, unlike denial_score which
            /// paradoxically rewards having unupgraded cells.
            ///
            /// Key fix: `- own_pot_w * own_upgrade_pot` instead of `+ own_pot_w * own_upgrade_pot`
            /// - High remaining potential (not upgraded) = LOWER score = incentive to upgrade
            /// - Low remaining potential (fully upgraded) = HIGHER score = reward
            ///
            /// Also adds frontier_potential from strategic_score for better territory evaluation.
            ...
            /// Ratio-aware evaluation: directly evaluates contest score formula
            /// Plus equalization (opponent balance), frontier growth, and leader denial.
            /// Parameters are passed in to avoid env_var reads in hot loop.
            ...
            /// Compute the frontier strength of the leading opponent.
            /// Higher values mean the leader has more expansion potential.
            ...
            /// Enriched territory evaluation: BFS connectivity + defensive strength + expansion potential.
