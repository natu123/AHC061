...
## 2026-02-16 Gear Shift（Exploreペースアップ宣言）
- 背景:
  - `SG-002` 時点で `ratio = 0.627108`、`top20_score = 69,401,964,303` に対して `top20_ratio = 0.669323` であり、上位帯との乖離が大きい状態です。
- 方針:
  - 直近 `3` ループは抜本探索を最優先し、微調整中心の試行を抑制します。
- 運用固定値（3ループ限定）:
  - 各ループで新規性の高い仮説を最低 `2` 件（推奨 `3` 件）実装して quick（`seed 0..19`）まで実施
  - full（`seed 0..99`）は quick 上位 `1` 件のみ実施
  - 微調整のみの試行は全体の `20%` 以下
  - `Exploit` は提出安全版の退行監視に限定（`5%`）
- 解除条件:
  - `ratio >= 0.70` を連続 `2` 回達成、または新規系統が full で `+2.0%` 以上を `2` 回再現した時点で通常配分へ戻す
