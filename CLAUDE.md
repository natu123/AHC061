...
## Score Optimization Discipline
- **フィルターは即打ち切り水準**: スコアフィルター・TLE制限・順位閾値は go/no-go ゲートである。通過できなければ そのアプローチを即打ち切り、別の戦略へ転換する。フィルター通過に向けてパラメータを微調整するのは目標の見失い
- **exploit/explore の区別を保つ**: 新戦略の探索 (explore) 中に、既存戦略のパラメータ微調整 (exploit) へ無意識に移行しない。同じ種類の変更を繰り返していたら explore から逸脱している
